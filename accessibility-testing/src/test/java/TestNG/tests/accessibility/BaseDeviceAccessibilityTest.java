 ...
 *
 * <p>Extend this class for page-specific or device-specific accessibility tests.
 * Override the {@link #getPageUrls()} method to provide your application's URLs.</p>
 *
 ...
 * <h2>Usage Example:</h2>
 * <pre>{@code
 * public class MyPageAccessibilityTest extends BaseDeviceAccessibilityTest {
 *
 *     @Override
 *     protected Map<String, String> getPageUrls() {
 *         Map<String, String> urls = new LinkedHashMap<>();
 *         urls.put("Home", "https://myapp.com/");
 *         urls.put("Login", "https://myapp.com/login");
 *         urls.put("Dashboard", "https://myapp.com/dashboard");
 *         return urls;
 *     }
 *
 *     @Test(dataProvider = "mobileDevices", groups = "mobile")
 *     public void testMobileAccessibility(Device device) {
 *         emulateDevice(device);
 *         PageResult result = checkPage(getPageUrls().get("Home"), "Home");
...
public abstract class BaseDeviceAccessibilityTest extends BaseAccessibilityTest {
    ...
    /**
     * Returns a map of page names to URLs for accessibility testing.
     *
     * <p>Override this method in your test class to provide application-specific URLs.</p>
     *
     * <h3>Example:</h3>
     * <pre>{@code
     * @Override
     * protected Map<String, String> getPageUrls() {
     *     Map<String, String> urls = new LinkedHashMap<>();
     *     urls.put("Home", Configuration.getInstance().getBaseUrl());
     *     urls.put("Login", Configuration.getInstance().getBaseUrl() + "/login");
     *     urls.put("Contact", Configuration.getInstance().getBaseUrl() + "/contact");
     *     return urls;
     * }
     * }</pre>
     *
     * @return Map of page name to URL
     */
    protected abstract Map<String, String> getPageUrls();
     ...
     * mvn test -Dtest=MyPageAccessibilityTest -Ddevice=IPHONE_12
     * mvn test -Dtest=MyPageAccessibilityTest -Ddevice=IPAD
     * mvn test -Dtest=MyPageAccessibilityTest -Ddevice=DESKTOP_FHD
     * </pre>
     *
    ...
    protected DeviceResult checkAllPages(Device device) {
        ...
        for (Map.Entry<String, String> entry : getPageUrls().entrySet()) {
            result.addPage(checkPage(entry.getValue(), entry.getKey()));
        }
    ...
    protected void logDeviceResult(DeviceResult result) {
        ...
        for (PageResult page : result.pages.values()) {
            ...
            String status = page.hasViolations() ? "!" : "+";
            log.info("  {} {}: {} Axe, {} Custom",
    ...
    protected void attachToReport(DeviceResult result) {
        ...
        try {
            ...
            String deviceType = result.device.isMobile() ? "Mobile" :
                               result.device.isTablet() ? "Tablet" : "Desktop";
            ...
            for (PageResult page : result.pages.values()) {
                try {
                    var node = test.createNode("Page: " + page.pageName);
