...
public class BaseTestTestNG {
    ...
    public void setup(@Optional("") String browser, @Optional("") String device) {
        ...
        try {
            ...
            // Route through Grid when grid.enabled=true so the browserType resolved
            // from the TestNG XML <parameter name="browser"> is requested on the Grid.
            WebDriver createdDriver = config.Configuration.getInstance().isGridEnabled()
                    ? DriverFactory.createRemoteDriver(browserType)
